const BOOKING_STORE = {
  ...
  openHour: 12,
  address: {
    en: "〒542-0082 Osaka, Chuo Ward, Shimanouchi, 1 Chome 19-15 Takayoshi Sakaisuji Building 2F",
    jp: "〒542-0082 大阪府大阪市中央区島之内1丁目19-15 高吉堺筋ビル2階",
    cn: "〒542-0082 大阪市中央区岛之内1丁目19-15 高吉堺筋大楼 2楼",
    tw: "〒542-0082 大阪市中央區島之內1丁目19-15 高吉堺筋大樓 2樓",
    kr: "〒542-0082 오사카시 주오구 시마노우치 1초메 19-15 다카요시사카스지 빌딩 2층"
  }
};
...
function buildCourseList(params) {
  ...
  if (typeof isAddonItem === "function" && isAddonItem(params.duration)) {
    return { list, selectedIndex: -1, hasCustom: false };
  }
...
function fullNameIfTruncated(option) {
  if (!option) return "";
  const full = optionLabel(option);
  return full !== shortOptionLabel(option) ? full : "";
}
...
function renderBooking() {
  ...
  function refresh() {
    ...
    syncSelectTip(courseSelect);
    additionalCourseFields.querySelectorAll("select").forEach(syncSelectTip);
